...
import EmojiCrushGame from './EmojiCrushGame'
import OriginalGameSidebar from '@/components/OriginalGameSidebar'
import { GAMES } from '@/data/games'
import { CATEGORIES } from '@/data/categories'

const game     = GAMES.find(g => g.id === 'emoji-crush')!
const category = CATEGORIES.find(c => c.id === game.category)

export const metadata: Metadata = {
  title: 'Emoji Crush — Match Emojis, Beat Levels | Antbreak',
  description:
    'Swap and match emojis to collect targets across 50 hand-crafted levels. ' +
    'Unlock bombs and rainbows, overcome obstacles, and earn 3 stars per level.',
}

export default function EmojiCrushPage() {
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      ...
      <nav aria-label="Breadcrumb" className="py-1">
        <ol className="flex flex-wrap items-center gap-x-1.5 gap-y-1 text-sm text-zinc-500 dark:text-zinc-400">
          ...
          <li aria-hidden="true"><ChevronRight className="h-3.5 w-3.5" /></li>
          <li className="font-medium text-zinc-900 dark:text-zinc-100">Emoji Crush</li>
        </ol>
      </nav>
      ...
      <div className="mt-6 space-y-4 pb-10">
        ...
        <section className="rounded-xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900">
          <h2 className="mb-3 text-lg font-semibold text-zinc-900 dark:text-zinc-100">About Emoji Crush</h2>
