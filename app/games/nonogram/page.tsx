...
const game     = GAMES.find(g => g.id === 'nonogram')!
const category = CATEGORIES.find(c => c.id === game.category)

export const metadata: Metadata = {
  title: 'Nonogram — Pixel Logic Puzzles | Antbreak',
  description:
    'Solve 100 handcrafted nonogram puzzles and reveal hidden pixel art. ' +
    'Use row and column number clues to fill in the grid. Sizes from 5×5 to 15×15.',
}
    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      ...
      <nav aria-label="Breadcrumb" className="py-1">
        <ol className="flex flex-wrap items-center gap-x-1.5 gap-y-1 text-sm text-zinc-500 dark:text-zinc-400">
          ...
          <li aria-hidden="true"><ChevronRight className="h-3.5 w-3.5" /></li>
          <li className="font-medium text-zinc-900 dark:text-zinc-100">Nonogram</li>
        </ol>
      </nav>
      ...
      <div className="mt-6 space-y-4 pb-10">
            ...
            <ul className="space-y-2.5">
              {game.tips.map((tip, i) => (
                <li key={i} className="flex items-start gap-2.5 text-sm leading-relaxed text-zinc-700 dark:text-zinc-300">
                  <span className="mt-0.5 select-none text-green-600 dark:text-green-400">◆</span>
                  {tip}
                </li>
              ))}
            </ul>
        ...
        <section className="rounded-xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900">
          <h2 className="mb-3 text-lg font-semibold text-zinc-900 dark:text-zinc-100">About Nonogram</h2>
