...
const game     = GAMES.find(g => g.id === 'image-memory-test')!
const category = CATEGORIES.find(c => c.id === game.category)

export const metadata: Metadata = {
  title: 'Image Memory Test — Free Brain Training Game | Antbreak',
  description:
    'Three memory training modes in one game. Memorize emoji sequences, match grid pairs, ' +
    'or tackle the daily flash challenge. Test and sharpen your visual memory.',
}

export default function ImageMemoryTestPage() {
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      ...
      <nav aria-label="Breadcrumb" className="py-1">
        <ol className="flex flex-wrap items-center gap-x-1.5 gap-y-1 text-sm text-zinc-500 dark:text-zinc-400">
          ...
          <li aria-hidden="true"><ChevronRight className="h-3.5 w-3.5" /></li>
          <li className="font-medium text-zinc-900 dark:text-zinc-100">Image Memory Test</li>
        </ol>
      </nav>
      ...
      <div className="mt-8 space-y-10 pb-10">
        ...
        {game.howToPlay.length > 0 && (
          <section id="how-to-play" className="rounded-xl border border-zinc-200 bg-zinc-50 p-6 dark:border-zinc-700 dark:bg-zinc-900">
        ...
        {game.tips.length > 0 && (
          <section className="rounded-xl border border-zinc-200 bg-zinc-50 p-6 dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <section>
          <h2 className="mb-3 text-xl font-bold text-zinc-900 dark:text-zinc-100">About Image Memory Test</h2>
