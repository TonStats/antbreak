import type { Game } from '@/types/game'
import { GAMEMONETIZE_GAMES } from './gamemonetize_full_catalog'

export const GAMES: Game[] = [
  {
    id: 'chess',
    name: 'Chess',
    slug: 'chess',
    category: 'original-games',
    description:
      'Play chess against an AI opponent with three difficulty levels. Easy for beginners, ' +
      'Medium for casual players, Hard for a real challenge. Track your moves, undo mistakes, ' +
      'and save your game automatically.',
    howToPlay: [
      'Select difficulty and choose to play as White or Black',
      'Click a piece to select it and see legal moves highlighted',
      'Click a highlighted square to move your piece',
      'Capture the opponent king to win — checkmate ends the game',
      'Use Undo Move to take back your last move if needed',
    ],
    tips: [
      'Control the center — place pawns and knights on d4, d5, e4, e5',
      'Develop your pieces early — get knights and bishops out before the queen',
      'King safety is critical — castle early to protect your king',
    ],
    thumbnail: '/thumbnails/chess.webp',
    gameUrl: '/games/chess',
    playCount: 0,
    featured: true,
    isOriginal: true,
    dateAdded: '2026-05-22',
    tags: ['chess', 'strategy', 'board', 'ai', 'classic'],
    iframeSettings: { width: 960, height: 600, allowFullscreen: true },
  },
  {
    id: 'sudoku',
    name: 'Sudoku',
    slug: 'sudoku',
    category: 'original-games',
    description:
      'Classic Sudoku puzzle game with three difficulty levels and a daily puzzle challenge. ' +
      'Fill the 9×9 grid so every row, column and 3×3 box contains the digits 1 to 9. ' +
      'Use notes mode, hints and undo to solve even the hardest puzzles.',
    howToPlay: [
      'Select a difficulty and choose Classic or Daily Puzzle mode',
      'Click any empty cell then use the number pad or keyboard to enter digits',
      'Every row, column and 3×3 box must contain 1 to 9 with no repeats',
      'Use Notes mode to pencil in candidate numbers',
      'Complete all 81 cells correctly to solve the puzzle',
    ],
    tips: [
      'Start with cells that can only be one number — called naked singles',
      'Use Notes mode to track candidates in difficult cells',
      'Save hints for cells where you are completely stuck',
    ],
    thumbnail: '/thumbnails/sudoku.webp',
    gameUrl: '/games/sudoku',
    playCount: 0,
    featured: true,
    isOriginal: true,
    dateAdded: '2026-05-22',
    tags: ['sudoku', 'puzzle', 'numbers', 'logic', 'daily'],
    iframeSettings: { width: 960, height: 700, allowFullscreen: true },
  },
  {
    id: 'typing-speed-test',
    name: 'Typing Speed Test',
    slug: 'typing-speed-test',
    category: 'original-games',
    description:
      'Test and improve your typing speed with real-time WPM tracking. Choose from Easy words, ' +
      'Medium sentences, Hard code snippets, or Expert prose. Track your accuracy and challenge your personal best.',
    howToPlay: [
      'Select your difficulty and timer duration',
      'Click the typing area or start typing to begin the timer',
      'Type the displayed text as accurately and quickly as possible',
      'See your WPM, accuracy and time left update in real time',
      'Review your results and grade when the timer ends',
    ],
    tips: [
      'Focus on accuracy first — errors hurt your WPM more than slowness',
      'Keep your eyes on the reference text, not your keyboard',
      'Start with Easy mode to build muscle memory before moving to Medium',
    ],
    thumbnail: '/thumbnails/typing-speed-test.webp',
    gameUrl: '/games/typing-speed-test',
    playCount: 0,
    featured: true,
    isOriginal: true,
    dateAdded: '2026-05-22',
    tags: ['typing', 'wpm', 'speed', 'keyboard', 'skill', 'words'],
    iframeSettings: { width: 960, height: 600, allowFullscreen: false },
  },
  {
    id: 'geography-challenge',
    name: 'Geography Challenge',
    slug: 'geography-challenge',
    category: 'original-games',
    description:
      'Test your world geography knowledge across five unique game modes. Identify flags, ' +
      'tap countries on a map, name capitals, and compare populations. A new daily challenge every day.',
    howToPlay: [
      'Choose a game mode and difficulty level',
      'Answer 10 questions as accurately as possible',
      'Each correct answer scores points',
      'Complete the daily challenge to build your streak',
      'Share your score to challenge friends',
    ],
    tips: [
      'Start with Easy difficulty to learn flag patterns',
      'Map Tap rewards close guesses even when wrong',
      'Daily Challenge uses all modes — prepare for variety',
    ],
    thumbnail: '/thumbnails/geography-challenge.webp',
    gameUrl: '/games/geography-challenge',
    playCount: 0,
    featured: true,
    isOriginal: true,
    source: 'original',
    dateAdded: '2026-05-23',
    tags: ['geography', 'flags', 'capitals', 'quiz', 'world'],
    iframeSettings: {
      width: 960,
      height: 600,
      allowFullscreen: true,
    },
  },
  {
    id: 'keyboard-shortcut-master',
    name: 'Keyboard Shortcut Master',
    slug: 'keyboard-shortcut-master',
    category: 'original-games',
    description:
      'Master keyboard shortcuts for Windows, Mac, Google Docs, VS Code, Excel and Photoshop. ' +
      'Three modes test your speed, recognition and software mastery.',
    howToPlay: [
      'Choose a mode and software category',
      'Press the correct key combination shown',
      'Build muscle memory through repetition',
      'Earn badges by completing all shortcuts',
      'Challenge your speed in Sprint mode',
    ],
    tips: [
      'Use Sprint mode daily to build muscle memory',
      'Start with Easy difficulty in each category',
      'The on-screen keyboard helps visualize keys',
    ],
    thumbnail: '/thumbnails/keyboard-shortcut-master.webp',
    gameUrl: '/games/keyboard-shortcut-master',
    playCount: 0,
    featured: true,
    isOriginal: true,
    source: 'original',
    dateAdded: '2026-05-24',
    tags: ['keyboard', 'shortcuts', 'productivity', 'typing', 'skills'],
    iframeSettings: { width: 960, height: 600, allowFullscreen: true },
  },
  {
    id: 'image-memory-test',
    name: 'Image Memory Test',
    slug: 'image-memory-test',
    category: 'original-games',
    description:
      'Three memory training modes test different aspects of your visual recall. ' +
      'Memorize emoji sequences, match grid pairs, or tackle the daily flash challenge. ' +
      'How sharp is your memory?',
    howToPlay: [
      'Choose Sequence, Grid Match, or Daily Flash mode',
      'In Sequence mode, memorize the emoji order then recreate it',
      'In Grid Match, flip cards to find matching emoji pairs',
      'In Daily Flash, study the image then answer questions about it',
      'Difficulty increases with each level — stay focused!',
    ],
    tips: [
      'Group emojis into mini-stories to memorize sequences faster',
      'In Grid Match, build a mental map of where each emoji is',
      'Daily Flash resets every day — keep your streak going',
    ],
    thumbnail: '/thumbnails/image-memory-test.webp',
    gameUrl: '/games/image-memory-test',
    playCount: 0,
    featured: true,
    isOriginal: true,
    source: 'original',
    dateAdded: '2026-05-26',
    tags: ['memory', 'matching', 'emoji', 'brain', 'daily'],
    iframeSettings: {
      width: 960,
      height: 600,
      aspectRatio: '16:9',
      paddingTop: '62.5%',
      allowFullscreen: true,
    },
  },
  {
    id: 'bible-quiz',
    name: 'Bible Quiz',
    slug: 'bible-quiz',
    category: 'original-games',
    description: 'Test your Bible knowledge across four unique modes covering both Old and New Testament.',
    howToPlay: [
      'Choose Quick Fire, Fill the Verse, Timeline, or Daily Challenge',
      'Select your difficulty and testament',
      'Answer each question within the time limit',
      'Read the explanation after each answer',
      'Build your daily streak with Daily mode',
    ],
    tips: [
      'Start with Easy difficulty to build confidence',
      'Fill the Verse has no timer — take your time',
      'Timeline tests sequence not just knowledge',
    ],
    thumbnail: '/thumbnails/bible-quiz.webp',
    gameUrl: '/games/bible-quiz',
    playCount: 0,
    featured: true,
    isOriginal: true,
    source: 'original',
    dateAdded: '2026-05-28',
    tags: ['bible', 'quiz', 'scripture', 'trivia', 'christian'],
    iframeSettings: {
      width: 960,
      height: 600,
      aspectRatio: '16:9',
      paddingTop: '62.5%',
      allowFullscreen: true,
    },
  },
  {
    id: 'general-knowledge-arena',
    name: 'General Knowledge Arena',
    slug: 'general-knowledge-arena',
    category: 'original-games',
    description:
      'Seven knowledge categories in one competitive trivia game. Test science, history, sports, ' +
      'entertainment, finance, health and arts knowledge across four unique game modes.',
    howToPlay: [
      'Choose Category Blitz, Arena, Sudden Death or Daily mode',
      'Select your difficulty level',
      'Answer each question within the time limit',
      'Read the explanation after each answer',
      'Earn category badges by scoring 80%+ in a category',
    ],
    tips: [
      'In Arena mode, save your strongest categories for later rounds',
      'Sudden Death rewards calm, methodical thinking over speed',
      'Daily mode covers all 7 categories — prepare broadly',
    ],
    thumbnail: '/thumbnails/general-knowledge-arena.webp',
    gameUrl: '/games/general-knowledge-arena',
    playCount: 0,
    featured: true,
    isOriginal: true,
    source: 'original',
    dateAdded: '2026-05-29',
    tags: ['trivia', 'knowledge', 'quiz', 'science', 'history', 'sports'],
    iframeSettings: {
      width: 960,
      height: 600,
      aspectRatio: '16:9',
      paddingTop: '62.5%',
      allowFullscreen: true,
    },
  },
  {
    id: 'puzzle-test',
    name: 'Puzzle Test',
    slug: 'puzzle-test',
    category: 'original-games',
    description:
      'Six types of visual and logical puzzles in one game. Number sequences, logic grids, odd one out, ' +
      'rebus decoding, and spatial challenges rotate so every question feels fresh.',
    howToPlay: [
      'Choose Classic, Speed Run, or Daily mode',
      'Each question is a different puzzle type — stay sharp',
      'Select the correct answer from the options shown',
      'Use hints sparingly — they cost points',
      'Complete the daily challenge to build your streak',
    ],
    tips: [
      'For sequences look at the differences between numbers, not just the values',
      'In odd one out, consider multiple categories before deciding',
      'Rebus puzzles use sound rather than spelling — say it out loud',
    ],
    thumbnail: '/thumbnails/puzzle-test.webp',
    gameUrl: '/games/puzzle-test',
    playCount: 0,
    featured: true,
    isOriginal: true,
    source: 'original',
    dateAdded: '2026-05-29',
    tags: ['puzzle', 'logic', 'sequence', 'brain', 'visual'],
    iframeSettings: {
      width: 960,
      height: 600,
      aspectRatio: '16:9',
      paddingTop: '62.5%',
      allowFullscreen: true,
    },
  },
  {
    id: 'riddle-test',
    name: 'Riddle Test',
    slug: 'riddle-test',
    category: 'original-games',
    description: 'Challenge your lateral thinking with 60 riddles across wordplay, logic, math, nature and what-am-I categories. Type your answer freely — no multiple choice.',
    howToPlay: [
      'Choose Classic, Speed, or Daily mode',
      'Read the riddle carefully',
      'Type your answer in the input box',
      'Use a hint if you are stuck',
      'Read the explanation after each answer',
    ],
    tips: [
      'Think literally and laterally — riddles use wordplay and double meanings',
      'Read every word carefully — the trick is often in the phrasing',
      'Use your hint before giving up — it points you in the right direction',
    ],
    thumbnail: '/thumbnails/riddle-test.webp',
    gameUrl: '/games/riddle-test',
    playCount: 0,
    featured: true,
    isOriginal: true,
    source: 'original',
    dateAdded: '2026-05-29',
    tags: ['riddles', 'logic', 'wordplay', 'thinking', 'brain'],
    iframeSettings: {
      width: 960,
      height: 600,
      aspectRatio: '16:9',
      paddingTop: '62.5%',
      allowFullscreen: true,
    },
  },
  {
    id: 'reaction-time-tester',
    name: 'Reaction Time Tester',
    slug: 'reaction-time-tester',
    category: 'original-games',
    description:
      'Measure your reaction speed across four precision tests. Click the color, aim at targets, ' +
      'recognise numbers, and match rhythm. See your time in milliseconds and compare to the average.',
    howToPlay: [
      'Choose a test type from the menu',
      'Follow the on-screen instructions for each test',
      'React as fast as possible when the stimulus appears',
      'Avoid false starts — they add a penalty',
      'Complete all rounds to see your average reaction time',
    ],
    tips: [
      'Relax your hand before each round — tension slows your reaction',
      'Focus on the center of the screen so you spot changes faster',
      'Morning sessions typically produce faster reactions than late at night',
    ],
    thumbnail: '/thumbnails/reaction-time-tester.webp',
    gameUrl: '/games/reaction-time-tester',
    playCount: 0,
    featured: true,
    isOriginal: true,
    source: 'original',
    dateAdded: '2026-05-31',
    tags: ['reaction', 'speed', 'timing', 'reflex', 'brain'],
    iframeSettings: {
      width: 960,
      height: 600,
      aspectRatio: '16:9',
      paddingTop: '62.5%',
      allowFullscreen: true,
    },
  },
  {
    id: 'emoji-crush',
    name: 'Emoji Crush',
    slug: 'emoji-crush',
    category: 'original-games',
    description:
      'Swap and match emojis to collect targets across 50 hand-crafted levels. ' +
      'Unlock bombs and rainbows, overcome locked and stone obstacles, and earn ' +
      'up to 3 stars per level.',
    howToPlay: [
      'Select a level from the story map and press Start Level',
      'Swap two adjacent emojis to create a line of 3 or more',
      'Match your target emojis to fill the collection bar',
      'Match 4 in a row to create a Bomb — match 5 or more for a Rainbow',
      'Complete all targets before your moves run out to win',
    ],
    tips: [
      'Prioritise matches that collect your target emojis over random matches',
      'Save bombs and rainbows for clearing locked or stone cell obstacles',
      'Cascading matches earn bonus score — set up chain reactions for big points',
    ],
    thumbnail: '/thumbnails/emoji-crush.webp',
    gameUrl: '/games/emoji-crush',
    playCount: 0,
    featured: true,
    isOriginal: true,
    source: 'original',
    dateAdded: '2026-06-02',
    tags: ['match3', 'puzzle', 'emoji', 'casual', 'levels'],
    iframeSettings: {
      width: 960,
      height: 600,
      aspectRatio: '16:9',
      paddingTop: '62.5%',
      allowFullscreen: true,
    },
  },
  {
    id: 'word-guess',
    name: 'Word Guess',
    slug: 'word-guess',
    category: 'original-games',
    description:
      'Guess the hidden word using letter-by-letter feedback. ' +
      'Words range from 4 to 10 letters across multiple difficulty levels. ' +
      'Race the clock in Speed Guess mode.',
    howToPlay: [
      'Type letters to fill in your guess',
      'Press Enter to submit your guess',
      'Green means correct letter and correct position',
      'Yellow means correct letter but wrong position',
      'Gray means the letter is not in the word at all',
    ],
    tips: [
      'Start with vowel-heavy words to map the vowels quickly',
      'Use what you know — green letters stay in position on every guess',
      'In Speed mode prioritise shorter words for quick wins',
    ],
    thumbnail: '/thumbnails/word-guess.webp',
    gameUrl: '/games/word-guess',
    playCount: 0,
    featured: true,
    isOriginal: true,
    source: 'original',
    dateAdded: '2026-06-02',
    tags: ['word', 'guess', 'vocabulary', 'puzzle', 'wordle'],
    iframeSettings: {
      width: 960,
      height: 600,
      aspectRatio: '16:9',
      paddingTop: '62.5%',
      allowFullscreen: true,
    },
  },
  {
    id: 'path-connector',
    name: 'Path Connector',
    slug: 'path-connector',
    category: 'original-games',
    description:
      'Draw paths to connect matching colored dots on a grid. ' +
      'Fill every cell without paths crossing. ' +
      '200 puzzle levels plus a reaction mode where speed matters.',
    howToPlay: [
      'Click a colored dot to start drawing',
      'Drag through the grid to connect it to its matching dot',
      'Fill every cell to complete the puzzle',
      'Paths cannot cross each other',
      'Click any path to clear it and redraw',
    ],
    tips: [
      'Start with dots in corners — they have fewer path options',
      'Count available cells to plan how paths must fill the grid',
      'In Reaction mode solve smaller puzzles first to build time',
    ],
    thumbnail: '/thumbnails/path-connector.webp',
    gameUrl: '/games/path-connector',
    playCount: 0,
    featured: true,
    isOriginal: true,
    source: 'original',
    dateAdded: '2026-06-03',
    tags: ['puzzle', 'path', 'logic', 'connect', 'spatial'],
    iframeSettings: {
      width: 960,
      height: 600,
      aspectRatio: '16:9',
      paddingTop: '62.5%',
      allowFullscreen: true,
    },
  },
  {
    id: 'nonogram',
    name: 'Nonogram',
    slug: 'nonogram',
    category: 'original-games',
    description:
      'Fill in the grid using number clues to reveal hidden pixel art. ' +
      'Numbers show groups of filled cells in each row and column. ' +
      '100 handcrafted puzzles from 5×5 to 15×15.',
    howToPlay: [
      'Numbers on rows and columns tell you which cells to fill',
      'Left-click to fill a cell black, right-click to mark a cell as empty',
      'A clue of "3 2" means a group of 3 filled cells, a gap, then a group of 2',
      'Complete the hidden image to solve the puzzle',
      'Use the X marks to track cells you know must be empty',
    ],
    tips: [
      'Start with rows or columns that have the largest clue numbers — they give the most information',
      'Use X marks to track cells you know must be empty',
      'If a row clue adds up to the row width, every cell in that row is filled',
    ],
    thumbnail: '/thumbnails/nonogram.webp',
    gameUrl: '/games/nonogram',
    playCount: 0,
    featured: true,
    isOriginal: true,
    source: 'original',
    dateAdded: '2026-06-03',
    tags: ['nonogram', 'picross', 'puzzle', 'logic', 'pixel'],
    iframeSettings: {
      width: 960,
      height: 600,
      aspectRatio: '16:9',
      paddingTop: '62.5%',
      allowFullscreen: true,
    },
  },
  {
    id: 'dodge-master',
    name: 'Dodge Master',
    slug: 'dodge-master',
    category: 'original-games',
    description:
      'Pure reflex survival game. Dodge incoming obstacles from all directions. ' +
      'Three modes test different aspects of your reaction speed. How long can you survive?',
    howToPlay: [
      'Move your character to dodge obstacles',
      'Use mouse or arrow keys to move',
      'Survive as long as possible',
      'Obstacles get faster as time goes on',
      'In Boss Rush survive each 30 second pattern',
    ],
    tips: [
      'Stay near the center — gives maximum escape routes in all directions',
      'Watch for gaps in obstacle patterns not just individual obstacles',
      'In Lane mode learn to read two moves ahead',
    ],
    thumbnail: '/thumbnails/dodge-master.webp',
    gameUrl: '/games/dodge-master',
    playCount: 0,
    featured: true,
    isOriginal: true,
    source: 'original',
    dateAdded: '2026-06-04',
    tags: ['dodge', 'reflex', 'arcade', 'survival', 'action'],
    iframeSettings: {
      width: 960,
      height: 600,
      aspectRatio: '16:9',
      paddingTop: '62.5%',
      allowFullscreen: true,
    },
  },
  // Self-hosted games (Cloudflare R2)
  {
    id: 'game-1',
    name: 'Game 1',
    slug: 'game-1',
    category: 'numbers-math',
    source: 'selfhosted',
    description: 'A reverse 2048 puzzle. Slide tiles to make matching numbers divide. Reach the tile with 1 to win.',
    howToPlay: [
      'Use arrow keys to move all tiles',
      'Matching tiles divide when they meet',
      'Reach the number 1 to win',
    ],
    tips: [],
    thumbnail: '/thumbnails/placeholder.webp',
    gameUrl: 'https://games.antbreak.com/1/index.html',
    playCount: 0,
    featured: false,
    isOriginal: false,
    dateAdded: '2026-06-10',
    tags: ['numbers', 'math', 'puzzle', 'tiles'],
    iframeSettings: { width: 960, height: 600, aspectRatio: '16:9', paddingTop: '62.5%', allowFullscreen: true },
  },
  {
    id: '10-minutes-till-dawn',
    name: '10 Minutes Till Dawn',
    slug: '10-minutes-till-dawn',
    category: 'action-arcade',
    source: 'selfhosted',
    description: 'Survive waves of monsters for 10 minutes. Shoot enemies, collect upgrades, and stay alive until dawn.',
    howToPlay: [
      'WASD to move your character',
      'Mouse to aim and shoot',
      'Collect upgrades to improve weapons',
      'Survive all 10 minutes to win',
    ],
    tips: [],
    thumbnail: '/thumbnails/placeholder.webp',
    gameUrl: 'https://games.antbreak.com/10-minutes-till-dawn/index.html',
    playCount: 0,
    featured: false,
    isOriginal: false,
    dateAdded: '2026-06-10',
    tags: ['survival', 'shooter', 'waves', 'action'],
    iframeSettings: { width: 960, height: 600, aspectRatio: '16:9', paddingTop: '62.5%', allowFullscreen: true },
  },
  {
    id: '2048',
    name: '2048',
    slug: '2048',
    category: 'numbers-math',
    source: 'selfhosted',
    description: 'Classic 2048 tile puzzle. Slide numbered tiles to merge matching pairs and reach the 2048 tile.',
    howToPlay: [
      'Use arrow keys to slide all tiles',
      'Matching tiles merge and double in value',
      'Reach the 2048 tile to win',
      'Plan carefully to avoid filling the board',
    ],
    tips: [],
    thumbnail: '/thumbnails/placeholder.webp',
    gameUrl: 'https://games.antbreak.com/2048/index.html',
    playCount: 0,
    featured: false,
    isOriginal: false,
    dateAdded: '2026-06-10',
    tags: ['numbers', 'math', 'puzzle', '2048'],
    iframeSettings: { width: 960, height: 600, aspectRatio: '16:9', paddingTop: '62.5%', allowFullscreen: true },
  },
  {
    id: 'ballistic-chickens',
    name: 'Ballistic Chickens',
    slug: 'ballistic-chickens',
    category: 'action-arcade',
    source: 'selfhosted',
    description: 'Launch chickens at targets in this physics-based action game. Aim carefully and cause maximum chaos.',
    howToPlay: [
      'Click or tap to aim and launch',
      'Hit all targets to complete each level',
      'Use physics to your advantage',
    ],
    tips: [],
    thumbnail: '/thumbnails/placeholder.webp',
    gameUrl: 'https://games.antbreak.com/ballistic-chickens/index.html',
    playCount: 0,
    featured: false,
    isOriginal: false,
    dateAdded: '2026-06-10',
    tags: ['physics', 'launch', 'action', 'chickens'],
    iframeSettings: { width: 960, height: 600, aspectRatio: '16:9', paddingTop: '62.5%', allowFullscreen: true },
  },
  {
    id: 'cars-simulator',
    name: 'Cars Simulator',
    slug: 'cars-simulator',
    category: 'sports-racing',
    source: 'selfhosted',
    description: 'Drive and explore in this open-world car simulator. Choose your vehicle and hit the road.',
    howToPlay: [
      'Arrow keys or WASD to drive',
      'Explore the environment freely',
      'Complete challenges to progress',
    ],
    tips: [],
    thumbnail: '/thumbnails/placeholder.webp',
    gameUrl: 'https://games.antbreak.com/cars-simulator/index.html',
    playCount: 0,
    featured: false,
    isOriginal: false,
    dateAdded: '2026-06-10',
    tags: ['cars', 'driving', 'simulator', 'racing'],
    iframeSettings: { width: 960, height: 600, aspectRatio: '16:9', paddingTop: '62.5%', allowFullscreen: true },
  },
  {
    id: 'core-ball',
    name: 'Core Ball',
    slug: 'core-ball',
    category: 'action-arcade',
    source: 'selfhosted',
    description: 'Shoot pins onto a rotating core without hitting existing pins. Precision timing is everything.',
    howToPlay: [
      'Click or tap to shoot a pin',
      'Do not hit any existing pins',
      'Clear all required pins to advance',
    ],
    tips: [],
    thumbnail: '/thumbnails/placeholder.webp',
    gameUrl: 'https://games.antbreak.com/core-ball/index.html',
    playCount: 0,
    featured: false,
    isOriginal: false,
    dateAdded: '2026-06-10',
    tags: ['precision', 'timing', 'pins', 'arcade'],
    iframeSettings: { width: 960, height: 600, aspectRatio: '16:9', paddingTop: '62.5%', allowFullscreen: true },
  },
  {
    id: 'draw-the-hill',
    name: 'Draw the Hill',
    slug: 'draw-the-hill',
    category: 'puzzle-logic',
    source: 'selfhosted',
    description: 'Draw hills and ramps to guide your vehicle to the finish line. Creative physics-based drawing puzzle.',
    howToPlay: [
      'Draw lines to create a path',
      'Guide your vehicle to the flag',
      'Avoid obstacles along the way',
    ],
    tips: [],
    thumbnail: '/thumbnails/placeholder.webp',
    gameUrl: 'https://games.antbreak.com/draw-the-hill/index.html',
    playCount: 0,
    featured: false,
    isOriginal: false,
    dateAdded: '2026-06-10',
    tags: ['drawing', 'physics', 'puzzle', 'hills'],
    iframeSettings: { width: 960, height: 600, aspectRatio: '16:9', paddingTop: '62.5%', allowFullscreen: true },
  },
  {
    id: 'go-ball',
    name: 'Go Ball',
    slug: 'go-ball',
    category: 'action-arcade',
    source: 'selfhosted',
    description: 'Guide a ball through obstacles and collect stars. Simple controls with challenging levels.',
    howToPlay: [
      'Tap or click to control the ball',
      'Avoid obstacles on the way',
      'Collect stars for bonus points',
    ],
    tips: [],
    thumbnail: '/thumbnails/placeholder.webp',
    gameUrl: 'https://games.antbreak.com/go-ball/index.html',
    playCount: 0,
    featured: false,
    isOriginal: false,
    dateAdded: '2026-06-10',
    tags: ['ball', 'obstacles', 'arcade', 'stars'],
    iframeSettings: { width: 960, height: 600, aspectRatio: '16:9', paddingTop: '62.5%', allowFullscreen: true },
  },
  {
    id: 'gravity-soccer',
    name: 'Gravity Soccer',
    slug: 'gravity-soccer',
    category: 'sports-racing',
    source: 'selfhosted',
    description: 'Score goals using gravity and physics. Tilt the field to roll the ball into the net.',
    howToPlay: [
      'Click or tap to change gravity',
      'Guide the ball into the goal',
      'Avoid your own goal',
    ],
    tips: [],
    thumbnail: '/thumbnails/placeholder.webp',
    gameUrl: 'https://games.antbreak.com/gravity-soccer/index.html',
    playCount: 0,
    featured: false,
    isOriginal: false,
    dateAdded: '2026-06-10',
    tags: ['soccer', 'gravity', 'physics', 'goals'],
    iframeSettings: { width: 960, height: 600, aspectRatio: '16:9', paddingTop: '62.5%', allowFullscreen: true },
  },
  {
    id: 'motox3m-pool',
    name: 'Moto X3M Pool Party',
    slug: 'motox3m-pool',
    category: 'sports-racing',
    source: 'selfhosted',
    description: 'Race your motorbike through water park obstacle courses. Perform stunts and reach the finish as fast as possible.',
    howToPlay: [
      'Arrow keys to accelerate and brake',
      'Balance your bike through obstacles',
      'Reach the finish line for 3 stars',
      'Faster completion earns more stars',
    ],
    tips: [],
    thumbnail: '/thumbnails/placeholder.webp',
    gameUrl: 'https://games.antbreak.com/motox3m-pool/index.html',
    playCount: 0,
    featured: false,
    isOriginal: false,
    dateAdded: '2026-06-10',
    tags: ['motorbike', 'racing', 'stunts', 'pool'],
    iframeSettings: { width: 960, height: 600, aspectRatio: '16:9', paddingTop: '62.5%', allowFullscreen: true },
  },
  {
    id: 'slope-ball',
    name: 'Slope Ball',
    slug: 'slope-ball',
    category: 'action-arcade',
    source: 'selfhosted',
    description: 'Control a ball rolling down an endless slope at high speed. Dodge obstacles and survive as long as possible.',
    howToPlay: [
      'Left and right arrow keys to steer',
      'Avoid red obstacles and gaps',
      'Survive as long as possible',
      'Speed increases over time',
    ],
    tips: [],
    thumbnail: '/thumbnails/placeholder.webp',
    gameUrl: 'https://games.antbreak.com/slope-ball/index.html',
    playCount: 0,
    featured: false,
    isOriginal: false,
    dateAdded: '2026-06-10',
    tags: ['slope', 'ball', 'endless', 'speed'],
    iframeSettings: { width: 960, height: 600, aspectRatio: '16:9', paddingTop: '62.5%', allowFullscreen: true },
  },
  {
    id: 'soccer-skills',
    name: 'Soccer Skills',
    slug: 'soccer-skills',
    category: 'sports-racing',
    source: 'selfhosted',
    description: 'Show off your soccer skills in this fast-paced football game. Dribble, shoot, and score goals.',
    howToPlay: [
      'Arrow keys or WASD to move',
      'Press space or click to shoot',
      'Score more goals than your opponent',
    ],
    tips: [],
    thumbnail: '/thumbnails/placeholder.webp',
    gameUrl: 'https://games.antbreak.com/soccer-skills/index.html',
    playCount: 0,
    featured: false,
    isOriginal: false,
    dateAdded: '2026-06-10',
    tags: ['soccer', 'football', 'skills', 'goals'],
    iframeSettings: { width: 960, height: 600, aspectRatio: '16:9', paddingTop: '62.5%', allowFullscreen: true },
  },
  {
    id: 'stickman-boost',
    name: 'Stickman Boost',
    slug: 'stickman-boost',
    category: 'action-arcade',
    source: 'selfhosted',
    description: 'Guide a stickman through extreme obstacle courses at high speed. React fast and reach the end alive.',
    howToPlay: [
      'Click or tap to jump',
      'Time your jumps to avoid obstacles',
      'Reach the end of each course',
    ],
    tips: [],
    thumbnail: '/thumbnails/placeholder.webp',
    gameUrl: 'https://games.antbreak.com/stickman-boost/index.html',
    playCount: 0,
    featured: false,
    isOriginal: false,
    dateAdded: '2026-06-10',
    tags: ['stickman', 'obstacles', 'speed', 'jump'],
    iframeSettings: { width: 960, height: 600, aspectRatio: '16:9', paddingTop: '62.5%', allowFullscreen: true },
  },
  {
    id: 'stickman-golf',
    name: 'Stickman Golf',
    slug: 'stickman-golf',
    category: 'sports-racing',
    source: 'selfhosted',
    description: 'Play mini golf with a stickman character across creative courses with unique obstacles.',
    howToPlay: [
      'Click and drag to aim your shot',
      'Release to set power and shoot',
      'Complete each hole in fewest shots',
    ],
    tips: [],
    thumbnail: '/thumbnails/placeholder.webp',
    gameUrl: 'https://games.antbreak.com/stickman-golf/index.html',
    playCount: 0,
    featured: false,
    isOriginal: false,
    dateAdded: '2026-06-10',
    tags: ['golf', 'stickman', 'minigolf', 'sports'],
    iframeSettings: { width: 960, height: 600, aspectRatio: '16:9', paddingTop: '62.5%', allowFullscreen: true },
  },
  {
    id: 'stickman-hook',
    name: 'Stickman Hook',
    slug: 'stickman-hook',
    category: 'action-arcade',
    source: 'selfhosted',
    description: 'Swing through levels using a grappling hook. Perfect your timing to chain swings and reach the finish.',
    howToPlay: [
      'Click or tap to shoot your hook',
      'Release to let go and swing forward',
      'Chain swings to reach the end',
    ],
    tips: [],
    thumbnail: '/thumbnails/placeholder.webp',
    gameUrl: 'https://games.antbreak.com/stickman-hook/index.html',
    playCount: 0,
    featured: false,
    isOriginal: false,
    dateAdded: '2026-06-10',
    tags: ['stickman', 'hook', 'swing', 'grapple'],
    iframeSettings: { width: 960, height: 600, aspectRatio: '16:9', paddingTop: '62.5%', allowFullscreen: true },
  },
  {
    id: 'x-trial-racing',
    name: 'X Trial Racing',
    slug: 'x-trial-racing',
    category: 'sports-racing',
    source: 'selfhosted',
    description: 'Race motorcycles through extreme trial courses. Balance your bike over obstacles to reach the finish line.',
    howToPlay: [
      'Arrow keys to accelerate and balance',
      'Do not let your rider fall off',
      'Complete each course to unlock the next',
    ],
    tips: [],
    thumbnail: '/thumbnails/placeholder.webp',
    gameUrl: 'https://games.antbreak.com/x-trial-racing/index.html',
    playCount: 0,
    featured: false,
    isOriginal: false,
    dateAdded: '2026-06-10',
    tags: ['trial', 'racing', 'motorcycle', 'balance'],
    iframeSettings: { width: 960, height: 600, aspectRatio: '16:9', paddingTop: '62.5%', allowFullscreen: true },
  },
  ...GAMEMONETIZE_GAMES,
]
